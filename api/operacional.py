...
def buscar_consulta(id_consulta):
    if not id_consulta:
        return jsonify({"error": "id nao informado"}), 400

    try:
        conn = SessionLocal()
        resultado  = conn.execute(text(
            """
                SELECT c.nome, retorno, horario, tipo_consulta, status, profissional_responsavel 
                FROM consultas JOIN cliente AS c ON consultas.id_cliente = c.id_cliente
...
def listar_lembretes():
    ...
    try:
        ...
        lembretes = [dict(row) for row in resultado.mappings()]
        if not lembretes:
            return jsonify({"error": "nenhum lembrete cadastrado"}), 404
...
def create_lembrete():
    ...
    data_disparo = data.get("data_disparo")
    horario_disparo = data.get("horario_disparo")
    ...
    try:
        ...
        conn.execute(text(
            """
                INSERT INTO lembrete (id_mensagem, data_disparo, horario_disparo, tipo_disparo, status, create_data) 
                VALUES(:id_mensagem, :data_disparo, :horario_disparo, :tipo_disparo, :status, :create_data);    
            """
        ),
        {
            "id_mensagem": id_mensagem,
        ...
        conn.commit()
        return jsonify({"mensagem": "lembrete criado com sucesso!",
        "lembrete": {
            "id_mensagem": id_mensagem,
            "data_disparo": data_disparo,
            "horario_disparo": horario_disparo,
...
def atualizar_lembrete(id_lembrete):
    ...
    campos_atualizar = ", ".join(f"{col} = :{col}" for col in campos)
    parametros = {**campos, "id_lembrete": id_lembrete, "update_data": datetime.now()}
    ...
    try:
        ...
        conn.commit()
        return jsonify({"mensagem": "lembrete atualizado com sucesso!",
        "lembrete": {
            **campos, "update_data": parametros["update_data"].isoformat()
        }}), 200
    finally:
        conn.close()
...
def deletar_lembrete(id_lembrete):
    ...
    try:
        ...
        conn.commit()
        return jsonify({"sucesso": "lembrete excluido com sucesso!"}), 200
    finally:
        conn.close()

#para comentar varias linhas Ctrl + /